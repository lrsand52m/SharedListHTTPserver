...
class HttpServer
{
    ...
    private:
        ...
        static bool HttpHandler(int sock)
        {
            ...
            cout<<"---------------------RecvHttpHeader is ok!\n";
            //解析http头部
            if (req.ParseHttpHeader(info) == false)
            {
                goto out;
            }
            cout<<"--------------------ParseHttpHeader is ok!\n";
            //判断请求是否是CGI请求
...
out:
    ...
    {}
        ...
        bool HttpServerInit(std::string ip, std::string port)
        {
            ...
            cout<<"-----get a _serv_sock :"<<_serv_sock<<"\n";
            int opt = 1;
            //设置地址复用
