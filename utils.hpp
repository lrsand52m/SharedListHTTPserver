...
class HttpRequest
{
    ...
    public:
        ...
        bool PathIsLegal(std::string& path, RequestInfo &info)
        {
            ...
            char tmp[MAX_PATH] = { 0 };
            //使用realpath函数将一个虚拟路径转化为物理路径的时候，就自动吧最后面的一��/去掉
...
class HttpResponse
{
    ...
    public:
        ...
        bool ProcessList(RequestInfo& info)
        {
            ...
            rsp_body += "<input type='submit' value='搜索' /></form>";  
            //<ol>是进行排序
            rsp_body += "<hr /><ol>";
            SendCData(rsp_body);
            //发送每一个文件的数据
            ...
            rsp_body = "</ol><hr /><a href=\"http://www.bitbug.net/\" target=\"_blank\"><img src=\"http://www.bitbug.net/mypagerank.php?style=9\" border=\"0\" alt=\"My Google PageRank\" /></a></body></html>";
            SendCData(rsp_body);
        ...
        bool ProcessCGI(RequestInfo& info)
        {
            ...
            rsp_body += "<a href=\"http://192.168.71.137:8888/\" title=\"点击返回\">点击返回</a>";
            rsp_body += "</body></html>";
            SendData(rsp_body);
